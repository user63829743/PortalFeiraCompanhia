...
function sponsorRow(sponsor) {
  const placementLabels = { home_bottom: "Home", home_split: "Home — grade", home_top: "Topo", feirantes_inline: "Feirantes", news_sidebar: "Notícias", news_sidebar_top: "Notícias — lateral superior", news_sidebar_bottom: "Notícias externas — lateral inferior", news_article_top: "Notícia interna — abaixo do título", news_article_sidebar: "Notícia interna — lateral superior", news_article_sidebar_bottom: "Notícia interna — lateral inferior", all: "Todas" };
  return `<tr><td><div class="sponsor-row-main"><img class="sponsor-thumb" src="${escapeHtml(sponsor.imageUrl)}" alt="Banner de ${escapeHtml(sponsor.name)}"><span><strong>${escapeHtml(sponsor.name)}</strong><small>${escapeHtml(placementLabels[sponsor.placement] || sponsor.placement || "Home")} · Slot ${escapeHtml(sponsor.slotKey || "1")}</small></span></div></td><td>${escapeHtml(placementLabels[sponsor.placement] || sponsor.placement || "Home")}</td><td>Slot ${escapeHtml(sponsor.slotKey || "1")} · 8s</td><td>${statusBadge(sponsor.status)}</td><td>${formatDate(sponsor.updatedAt || sponsor.createdAt)}</td><td class="actions-cell"><button class="details-button" type="button" data-edit-sponsor="${sponsor.id}">Editar</button><button class="text-danger" type="button" data-delete-sponsor="${sponsor.id}">Excluir</button></td></tr>`;
}
...
function renderSponsorPreview(existingUrl = "") {
  ...
  if (!gallery) return;
  const file = sponsorSelectedFiles[0];
  const url = file ? URL.createObjectURL(file) : existingUrl;
  gallery.innerHTML = url ? `<img src="${escapeHtml(url)}" alt="Pré-visualização do banner do slot">` : "";
  gallery.classList.toggle("hidden", !url);
}
...
function syncSponsorFileInput() {
  ...
  const file = sponsorSelectedFiles[0];
  if (count) count.textContent = file ? `1 banner selecionado: ${file.name}` : "Nenhum banner selecionado";
  if (list) list.innerHTML = file ? `<div class="sponsor-file-item"><span>${escapeHtml(file.name)}</span></div>` : "";
}

function updateSponsorSlotVisibility() {
  const placement = $("#sponsorForm")?.elements?.placement?.value;
  const slotField = $("#sponsorSlotField");
  const rotationField = $("#sponsorRotationField");
  const slotAndRotationPositions = new Set(["home_split", "news_sidebar_top", "news_sidebar_bottom", "news_article_sidebar", "news_article_sidebar_bottom"]);
  const supportsSlotAndRotation = slotAndRotationPositions.has(placement);
  if (slotField) {
    const showSlot = supportsSlotAndRotation;
    slotField.classList.toggle("hidden", !showSlot);
    slotField.querySelector("select").disabled = !showSlot;
    if (!showSlot) slotField.querySelector("select").value = "1";
  }
  if (rotationField) {
    rotationField.classList.toggle("hidden", !supportsSlotAndRotation);
    rotationField.querySelector("input").disabled = !supportsSlotAndRotation;
    if (!supportsSlotAndRotation) rotationField.querySelector("input").value = "8";
  }
}
...
function openSponsorModal(sponsor = null) {
  ...
  form.elements.slotKey.value = sponsor?.slotKey || "1";
  form.elements.rotationSeconds.value = sponsor?.rotationSeconds ?? 8;
  updateSponsorSlotVisibility();
...
async function saveSponsor(event) {
  ...
  delete data.id;
  delete data.image;
  if (!id && !file) { $("#sponsorFormMessage").textContent = "Selecione o banner deste slot."; $("#sponsorFormMessage").className = "form-message error"; return; }
  if (file && file.size > 5 * 1024 * 1024) { $("#sponsorFormMessage").textContent = "O banner deve ter no máximo 5 MB."; $("#sponsorFormMessage").className = "form-message error"; return; }
  const button = form.querySelector('button[type="submit"]');
  button.disabled = true;
  try {
    if (file) data.imageDataUrl = await fileToDataUrl(file);
    if (id) await api(`/api/sponsors/${id}`, { method: "PATCH", body: JSON.stringify(data) });
    else await api("/api/sponsors", { method: "POST", body: JSON.stringify(data) });
    closeSponsorModal(); showToast(id ? "Slot de banner atualizado com sucesso." : "Banner salvo no slot com sucesso."); await loadSponsors();
...
$("#sponsorForm").elements.placement.addEventListener("change", updateSponsorSlotVisibility);
...
$("#sponsorImageInput").addEventListener("change", (event) => {
  sponsorSelectedFiles = event.currentTarget.files[0] ? [event.currentTarget.files[0]] : [];
  syncSponsorFileInput();
