...
async function getActiveSponsors() {
  if (!SUPABASE_URL || !SUPABASE_SECRET_KEY) return [];
  const headers = { apikey: SUPABASE_SECRET_KEY, Authorization: `Bearer ${SUPABASE_SECRET_KEY}` };
  const advancedSelect = 'id,name,image_url,target_url,display_order,status,starts_at,ends_at,placement,campaign_group,slot_key,display_mode,rotation_seconds';
  ...
  if (!response.ok && response.status === 400) response = await fetch(`${SUPABASE_URL}/rest/v1/sponsor_banners?select=id,name,image_url,target_url,display_order,status,starts_at,ends_at,placement,campaign_group,display_mode,rotation_seconds&order=display_order.asc,created_at.desc`, { headers, signal: AbortSignal.timeout(8000) });
  if (!response.ok) throw new Error(`Supabase patrocinadores indisponível (${response.status}).`);
  const now = Date.now();
  return (await response.json()).filter((item) => item.status === 'active' && (!item.starts_at || new Date(item.starts_at).getTime() <= now) && (!item.ends_at || new Date(item.ends_at).getTime() >= now)).slice(0, 16).map((item) => ({ id: item.id, name: item.name, imageUrl: item.image_url, targetUrl: item.target_url || '', displayOrder: Number(item.display_order || 0), placement: item.placement || 'home_bottom', campaignGroup: item.campaign_group || 'default', slotKey: String(item.slot_key || '1'), displayMode: 'rotate', rotationSeconds: 8 }));
}
