...
function newsSponsorSlots(sponsors = []) {
  ...
  const sortSlots = (items) => items.sort((a, b) => Number(a.slotKey || 1) - Number(b.slotKey || 1) || Number(a.displayOrder || 0) - Number(b.displayOrder || 0));
  const top = sortSlots(available.filter((sponsor) => ['news_sidebar_top', 'news_sidebar', 'all'].includes(sponsor.placement))).slice(0, 4);
  const bottom = sortSlots(available.filter((sponsor) => ['news_sidebar_bottom', 'all'].includes(sponsor.placement))).slice(0, 4);
  const articleTop = sortSlots(sponsors.filter((sponsor) => ['news_article_top', 'all'].includes(sponsor.placement))).slice(0, 4);
  const articleSidebar = sortSlots(sponsors.filter((sponsor) => ['news_article_sidebar', 'all'].includes(sponsor.placement))).slice(0, 4);
  const articleSidebarBottom = sortSlots(sponsors.filter((sponsor) => ['news_article_sidebar_bottom', 'all'].includes(sponsor.placement))).slice(0, 4);
  return { top, bottom, articleTop, articleSidebar, articleSidebarBottom };
}

function mountRotatingSponsor(container, sponsors = []) {
  if (!container) return;
  if (container.__sponsorTimer) { clearInterval(container.__sponsorTimer); container.__sponsorTimer = null; }
  const items = sponsors.filter((sponsor) => sponsor?.imageUrl).slice(0, 4);
  if (!items.length) { container.innerHTML = '<span>Espaço para anúncio</span>'; return; }
  let index = 0;
  const paint = () => { container.innerHTML = sponsorMarkup(items[index]) || '<span>Espaço para anúncio</span>'; };
  paint();
  if (items.length > 1) container.__sponsorTimer = setInterval(() => { index = (index + 1) % items.length; paint(); }, 8000);
}
...
async function loadBanner() {
  ...
  try {
    ...
    mountRotatingSponsor(primary, slots.top);
    if (slots.bottom.length) { secondary.hidden = false; mountRotatingSponsor(secondary, slots.bottom); }
    else { secondary.hidden = true; secondary.innerHTML = ''; }
  } catch {
    mountRotatingSponsor(primary, []);
...
async function loadArticleBanners() {
  ...
  try {
    ...
    const { sponsors = [] } = await response.json();
    const slots = newsSponsorSlots(sponsors);
    mountRotatingSponsor(top, slots.articleTop);
    mountRotatingSponsor(lateral, slots.articleSidebar);
    if (slots.articleSidebarBottom.length) { lateralSecondary.hidden = false; mountRotatingSponsor(lateralSecondary, slots.articleSidebarBottom); }
    else { lateralSecondary.hidden = true; lateralSecondary.innerHTML = ''; }
  } catch {
    mountRotatingSponsor(top, []);
    mountRotatingSponsor(lateral, []);
