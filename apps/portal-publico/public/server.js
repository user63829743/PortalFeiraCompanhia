...
function renderSponsors(sponsors, grid, section) {
  const split = sponsors.filter((sponsor) => sponsor.placement === 'home_split').sort((a, b) => Number(a.slotKey || 1) - Number(b.slotKey || 1) || Number(a.displayOrder || 0) - Number(b.displayOrder || 0)).slice(0, 4);
  const visible = split.length ? split : sponsors.filter((sponsor) => sponsor.placement === 'home_bottom').slice(0, 1);
