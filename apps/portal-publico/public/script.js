...
function renderSponsors(sponsors, grid, section) {
  ...
  const splitGroup = sponsors
    .filter((sponsor) => sponsor.placement === 'home_split')
    .sort((a, b) => Number(a.slotKey || 1) - Number(b.slotKey || 1) || Number(a.displayOrder || 0) - Number(b.displayOrder || 0))
    .slice(0, 4);
  if (splitGroup.length) return renderSponsorItems(splitGroup, grid, section, 4);
  const single = sponsorsForPlacement(sponsors, 'home_bottom').slice(0, 1);
  return renderSponsorItems(single, grid, section, 1);
}
